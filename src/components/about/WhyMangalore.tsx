...
export default function WhyMangalore() {
  ...
  const content = settings.about.why
      ...
      <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
        <div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {content.points.filter((point) => point.active !== false).slice().sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0)).map((point, index) => (
        ...
        <motion.div initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.45 }}>
          <Card variant="image" className="overflow-hidden border-white/15 p-0">
            <figure className="relative">
              <div role="img" aria-label={content.imageTitle} className={`aspect-[4/5] bg-cover bg-center ${beachVisual?.imageClass ?? ''}`} style={content.imageUrl ? { backgroundImage: `url('${content.imageUrl}')` } : undefined} />
