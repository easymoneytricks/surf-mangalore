...
import { aboutCommunityParagraphs, aboutImagePlaceholders } from '../../data/about'

export default function CommunitySection() {
  const communityVisual = aboutImagePlaceholders.find((item) => item.id === 'community')
      ...
      <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <div>
          ...
          <h2 className="mt-4 font-[var(--font-heading)] text-[clamp(1.8rem,3.2vw,2.5rem)] font-[var(--font-semibold)] leading-[1.08] tracking-[var(--letter-tight)] text-[var(--color-text)]">
            More than a surf school. A coastal circle of people.
          </h2>
          <div className="mt-5 space-y-4 text-base leading-8 text-[var(--color-text-secondary)]">
            {aboutCommunityParagraphs.map((paragraph) => (
        ...
        <motion.div initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.45 }}>
          <Card variant="image" className="overflow-hidden border-white/15 p-0">
            <figure className="relative">
              <div role="img" aria-label={communityVisual?.title ?? 'Community culture'} className={`aspect-[4/5] bg-cover bg-center ${communityVisual?.imageClass ?? ''}`} />
              ...
              <figcaption className="absolute bottom-5 left-5 right-5 rounded-[1.25rem] border border-white/12 bg-[rgba(4,19,27,0.58)] p-4 backdrop-blur-lg">
                <p className="text-lg font-semibold text-white">{communityVisual?.title}</p>
                <p className="mt-2 text-sm leading-7 text-[var(--color-text-secondary)]">{communityVisual?.description}</p>
              </figcaption>
            </figure>
          </Card>
        </motion.div>
