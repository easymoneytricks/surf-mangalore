...
export default function ExperienceHero() {
  ...
  const content = settings.experiencePage.hero
  return (
      ...
      <div className="relative z-10 grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-10">
        ...
        <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.08 }}>
          <Card variant="image" className="overflow-hidden border-white/15 p-0">
            <div className="relative aspect-[4/5] bg-[linear-gradient(135deg,rgba(122,214,209,0.16),rgba(255,143,74,0.16)),url('/images/placeholders/ocean.svg')] bg-cover bg-center" style={content.imageUrl ? { backgroundImage: `url('${content.imageUrl}')` } : undefined}>
              ...
              <div className="absolute bottom-5 left-5 right-5 rounded-[1.5rem] border border-white/12 bg-[rgba(4,19,27,0.6)] p-5 backdrop-blur-xl">
                <p className="text-[0.72rem] uppercase tracking-[0.3em] text-[var(--color-primary)]">{content.whatToExpectLabel}</p>
                <p className="mt-2 text-sm leading-7 text-[var(--color-text-secondary)]">
                  {content.whatToExpectText}
                </p>
              </div>
            </div>
          </Card>
