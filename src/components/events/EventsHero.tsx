...
import { eventHeroStats } from '../../data/events'
    ...
    <section className="relative isolate overflow-hidden rounded-[2.4rem] border border-white/10 bg-[linear-gradient(135deg,rgba(5,21,31,0.98),rgba(8,32,46,0.92))] px-5 py-6 shadow-[0_40px_100px_rgba(4,19,27,0.34)] sm:px-8 sm:py-8 lg:px-10 lg:py-10 xl:px-12 xl:py-12">
      ...
      <div className="relative z-10 grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-10">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <Badge tone="accent">Surf events · community · competition</Badge>
          <h1 className="mt-5 max-w-[11ch] font-[var(--font-heading)] text-[clamp(2.7rem,5vw,5rem)] font-[var(--font-display)] leading-[0.88] tracking-[var(--letter-tight)] text-[var(--color-text)]">
            Join the rhythm of the coast.
          </h1>
          <p className="mt-6 max-w-[36rem] text-base leading-8 text-[var(--color-text-secondary)] sm:text-lg">
            Our events are designed to feel like more than a calendar entry. They are a social surf experience where adventure, celebration, and coastal community come together in one place.
          </p>

          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Button variant="primary" size="lg" onClick={() => navigateTo('/contact')}>
              Join an event
            </Button>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {eventHeroStats.map((stat) => (
              <Card key={stat.label} variant="glass" className="border-white/12 px-4 py-4">
        ...
        <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.08 }}>
          <Card variant="image" className="overflow-hidden border-white/15 p-0">
            <div className="relative aspect-[4/5] bg-[linear-gradient(135deg,rgba(122,214,209,0.16),rgba(255,143,74,0.16)),url('/images/placeholders/events.svg')] bg-cover bg-center">
              ...
              <div className="absolute bottom-5 left-5 right-5 rounded-[1.5rem] border border-white/12 bg-[rgba(4,19,27,0.6)] p-5 backdrop-blur-xl">
                <p className="text-[0.72rem] uppercase tracking-[0.3em] text-[var(--color-primary)]">What events feel like</p>
                <p className="mt-2 text-sm leading-7 text-[var(--color-text-secondary)]">
                  A premium coastal gathering with guided surf, cheering from the shoreline, and the kind of atmosphere people want to post about and return to.
                </p>
              </div>
            </div>
          </Card>
